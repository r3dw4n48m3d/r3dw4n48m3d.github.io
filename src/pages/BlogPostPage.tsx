...
const BlogPostPage = () => {
  ...
  const renderContent = (item: BlogContent, index: number) => {
    switch (item.type) {
      case 'paragraph':
        return (
          <p key={index} className="text-green-300 leading-relaxed mb-6 font-mono">
            {item.content}
          </p>
        );
        ...
        return (
          <div key={index} className="mb-6 relative group">
            <div className="flex items-center justify-between bg-gray-900 px-4 py-2 rounded-t border border-b-0 border-green-900/50">
              <span className="text-green-600 font-mono text-sm">{item.language}</span>
              <button
        ...
        return (
          <ul key={index} className="mb-6 space-y-2">
            {item.items?.map((listItem, i) => (
              <li key={i} className="flex items-start gap-3 text-green-300 font-mono">
        ...
        <article className="pt-24 pb-16 px-4">
          <div className="max-w-4xl mx-auto">
            ...
            <div className="flex items-center gap-2 text-sm font-mono mb-8">
              <Link to="/" className="text-green-600 hover:text-cyan-400 transition-colors">
                ~
              </Link>
              <span className="text-green-800">/</span>
              <Link to="/blog" className="text-green-600 hover:text-cyan-400 transition-colors">
                blog
              </Link>
              <span className="text-green-800">/</span>
              ...
              <div className="flex flex-wrap items-center gap-4 text-sm font-mono text-green-600 mb-8">
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    ...
                    <div className="text-green-700 text-xs">Security Researcher</div>
                  </div>
                </div>
                <span className="text-green-800">|</span>
              ...
              <aside className="hidden lg:block lg:w-64 flex-shrink-0">
                <div className="sticky top-24">
                  ...
                  <div className="mt-4 bg-black/50 border border-green-900/50 rounded-lg p-4">
                    <h3 className="text-cyan-400 font-mono text-sm mb-4">Share</h3>
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="mt-16 p-6 bg-black/50 border border-green-900/50 rounded-lg">
                  <div className="flex items-start gap-4">
                    ...
                    <div>
                      <h3 className="text-cyan-400 font-bold text-lg">@{post.author}</h3>
                      <p className="text-green-600 font-mono text-sm mb-3">
                        Security Researcher | Bug Bounty Hunter | CTF Player
                      </p>
                      <p className="text-green-500 text-sm">
                        Passionate about breaking things to make them secure. Sharing knowledge through write-ups and tutorials.
                      </p>
                    </div>
                  </div>
                </div>
                ...
                <div className="mt-12 grid md:grid-cols-2 gap-4">
                  {prevPost && (
                    ...
                    >
                      <div className="text-green-600 text-sm font-mono mb-2">← Previous Post</div>
                  ...
                  {nextPost && (
                    ...
                    >
                      <div className="text-green-600 text-sm font-mono mb-2">Next Post →</div>
                      <div className="text-green-400 group-hover:text-cyan-400 transition-colors line-clamp-2">
                        {nextPost.title}
                      </div>
                    </Link>
